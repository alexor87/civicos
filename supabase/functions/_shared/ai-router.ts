/**
 * AI Router — Shared module for Supabase Edge Functions
 *
...
async function resolveConfig(
  ...
  // Try campaign-specific config first, then tenant default
  ...
  const config = configs?.find(c => c.campaign_id === campaignId) ?? configs?.find(c => c.campaign_id === null)

  if (!config) {
    throw new AiNotConfiguredError(campaignId)
  }

  // Decrypt key via DB function
...
export async function callAI(
  ...
  if (config.provider === 'anthropic') {
    ...
    // OpenAI, Mistral, Groq — all use OpenAI-compatible API
