  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center gap-2 bg-slate-50 rounded-lg p-3 border border-slate-200">
        <span className="flex-1 text-sm text-slate-600 truncate">{referralLink}</span>
        <button
          onClick={handleCopy}
          className="shrink-0 p-2 rounded-md hover:bg-slate-200 transition-colors"
          title="Copiar enlace"
        >
          {copied ? (
      ...
      <button
        onClick={handleCopy}
        className="flex items-center justify-center gap-2 w-full py-3 rounded-lg border border-slate-300 text-slate-700 font-medium text-sm hover:bg-slate-50 transition-colors"
      >
